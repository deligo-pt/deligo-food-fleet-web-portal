...
import { Search, Clock, MapPin, Navigation,  Eye, PackageCheck, SignalHigh } from "lucide-react";
      ...
      <div className="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 flex items-center gap-3">
            <PackageCheck size={20} /> On-The-Way Deliveries
          </h1>
          <p className="text-sm text-gray-600">Live tracking for active delivery orders across Portugal.</p>
        </div>
        ...
        <div className="flex items-center bg-white rounded-lg shadow-sm overflow-hidden w-full sm:w-80">
          ...
          <input
            ...
            placeholder="Search partner, customer, address, order ID…"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((d) => (
          ...
          >
            ...
            <div className="mt-2 text-xs text-gray-500">Time elapsed: {d.timeElapsed} • Speed: {d.speed}</div>
          </div>
        ))}
      </div>
      ...
      {active && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <aside className="ml-auto w-full sm:w-[460px] bg-white shadow-2xl rounded-l-2xl p-6 overflow-y-auto animate-slide-in">
            <h3 className="text-lg font-semibold">On-The-Way #{active.id}</h3>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">Speed: {active.speed}</div>
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              <button onClick={() => setActive(null)} className="px-4 py-2 rounded-md text-white" style={{ backgroundColor: DELIGO }}>Close</button>
              <button className="px-4 py-2 rounded-md border bg-white flex items-center gap-2"><Eye size={14}/> View Partner</button>
            </div>
          </aside>
        </div>
      )}
