...
import { Search, Eye, RefreshCcw, AlertTriangle, FileText } from "lucide-react";
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 flex items-center gap-2">
            <AlertTriangle className="text-red-500" /> Suspended Delivery Partners
          </h1>
          <p className="text-sm text-gray-600">Fleet Manager can preview & reinstate suspended couriers.</p>
        </div>
        ...
        {filtered.map((p) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div className={`h-12 w-12 rounded-full flex items-center justify-center text-white text-sm font-semibold ${p.avatarColor}`}>{p.name[0]}</div>
              <div>
            ...
            <div className="mt-4 text-sm text-gray-700 space-y-1">
              ...
              <p><strong>Location:</strong> {p.city}</p>
              <p><strong>Vehicle:</strong> {p.vehicle}</p>
              <p className="text-xs text-gray-500">Suspended: {new Date(p.suspendedAt).toLocaleDateString()}</p>
            </div>
      ...
      {active && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <aside className="ml-auto w-full sm:w-96 bg-white shadow-2xl rounded-l-2xl p-6 overflow-y-auto animate-slide-in">
            ...
            <p className="text-xs text-gray-500">{active.id} • {active.city}</p>

            <div className="mt-6 space-y-4">
              <div className="flex items-center gap-4">
                <div className={`h-14 w-14 rounded-full flex items-center justify-center text-white text-xl ${active.avatarColor}`}>{active.name[0]}</div>
                <div>
              ...
              <div className="p-3 rounded-lg bg-gray-50">
                ...
                <p className="font-medium">{new Date(active.suspendedAt).toLocaleString()}</p>
              </div>
      ...
      <style jsx>{`
        ...
        .animate-slide-in {
          animation: slide-in 220ms cubic-bezier(.2,.9,.2,1) both;
        }
      `}</style>
    </div>
  );
