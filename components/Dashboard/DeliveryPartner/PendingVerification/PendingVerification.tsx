...
export default function PendingVerification({
  ...
  //       documents: [],
  //       avatarColor: ["bg-rose-200", "bg-pink-200", "bg-amber-200"][
    ...
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">
            Delivery Partners — Pending Verification
          </h1>
          <p className="text-sm text-gray-600">
            Fleet manager can Preview and Add partners only.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow ring-1 ring-gray-100 overflow-hidden">
        <div className="w-full overflow-x-auto">
          <table className="min-w-full table-fixed divide-y divide-gray-200">
            <thead className="bg-white">
              <tr>
                <th className="px-4 py-3 text-left text-sm text-gray-500 w-56">
                  Partner
                </th>
                <th className="px-4 py-3 text-left text-sm text-gray-500 hidden md:table-cell">
                  Contact
                </th>
                <th className="px-4 py-3 text-left text-sm text-gray-500">
                  City
                </th>
                <th className="px-4 py-3 text-left text-sm text-gray-500 hidden lg:table-cell">
                  Vehicle
                </th>
                <th className="px-4 py-3 text-right text-sm text-gray-500 w-28">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {partnersResult?.data?.map((p) => (
                <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-semibold text-white bg-[#DC3173]`}
                        aria-hidden
                      >
                  ...
                  <td className="px-4 py-4 hidden lg:table-cell text-sm">
                    {p?.vehicleInfo?.vehicleType || "N/A"}
                  </td>

                  <td className="px-4 py-4 text-right">
                    <button
                      onClick={() =>
                        router.push(`/agent/delivery-partners/${p.userId}`)
                      }
        ...
        {/* <div className="px-4 py-3 flex items-center justify-between text-sm text-gray-600">
          <div>
            Showing <strong>{filtered.length}</strong> partners
          </div>
          ...
          <aside className="ml-auto w-full sm:w-96 bg-white shadow-2xl rounded-l-2xl p-6 overflow-y-auto animate-slide-in">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {activePartner.id} • {activePartner.city}
                </p>
              </div>
            ...
            <div className="mt-6 space-y-4">
              <div className="flex items-center gap-4">
                <div
                  className={`h-14 w-14 rounded-full flex items-center justify-center text-white ${activePartner.avatarColor}`}
                >
                  {activePartner.name
                    .split(" ")
                    .map((n) => n[0])
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-gray-400">
                    {activePartner.documents.length} files
                  </div>
                </div>
              ...
              <div>
                ...
                <ul className="mt-2 text-xs text-gray-500 space-y-2">
                  <li>
                    Joined: {new Date(activePartner.joinedAt).toLocaleString()}
                  </li>
                  <li>Trips: —</li>
                  <li>Last login: —</li>
                </ul>
      ...
      <style jsx>{`
        ...
        .animate-slide-in {
          animation: slide-in 180ms cubic-bezier(0.2, 0.9, 0.2, 1) both;
        }
      `}</style>
    </div>
  );
