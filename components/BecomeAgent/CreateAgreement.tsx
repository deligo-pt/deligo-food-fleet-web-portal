...
import { ArrowLeftCircle, FileText, Save, User } from "lucide-react";
...
import { fleetAgreementSchema, TFleetAgreementForm } from "@/validations/agreement/agreement.validation";
...
export default function CreateAgreement({ fleetId }: IProps) {
    const { t } = useTranslation();
    const router = useRouter();

    const form = useForm<TFleetAgreementForm>({
        resolver: zodResolver(fleetAgreementSchema),
        defaultValues: {
            signatoryType: "SELF",
    ...
    const onSubmit = async (data: TFleetAgreementForm) => {
        const toastId = toast.loading("Creating fleet agreement...");
        ...
        ;
        const result = await createAgreement(fleetId, payload);
        ...
        if (result?.success) {
            ...
            router.push(`/become-agent/agreement/sign?agreementId=${encodeURIComponent(result?.data?._id)}`);
            return;
        }
            ...
            <div className="max-w-3xl mx-auto">
                <Card className="rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
                    <div className="relative p-0">
                        ...
                        >
                            <ArrowLeftCircle className="w-4 h-4" /> {t("go_back")}
                        </Button>
                    </div>
